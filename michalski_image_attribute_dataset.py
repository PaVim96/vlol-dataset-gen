...
class MichalskiAttributeImageDataset(MichalskiDataset): 
    ''' Dataset for the Michalski train dataset with Images and Attributes as output and the direction as the labels
        parameters:
    ...
    def __init__(self, fixed_car_size=4, **kwargs): 
    ...
    def __getitem__(self, item):
        ...
        X = self.norm(image)
        attributes = self.get_attributes_fixed_size(item)
        y = self.get_direction(item)

        return X, attributes, y 
